...
const SignUpPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Submit to backend
      const response = await fetch('https://auth-service-ancient-frost-8646.fly.dev/auth/register', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setErrors(data.errors?.map((err: any) => err.msg) || [data.error || 'Registration failed']);
      }
    } catch (error) {
